#!/usr/bin/env bash
#
# TestSolver Seven 1.4.2, a fake solver for exercising the pipeline.
#
...
# exactly testsolver7 into $SOLVER_BIN_DIR and exits 0.
#
set -euo pipefail
...
cat > "$SOLVER_BIN_DIR/testsolver7" <<'SOLVER'
#!/usr/bin/env bash
set -uo pipefail

case "${1:-}" in
  --name)    echo "TestSolver Seven" ; exit 0 ;;
  --version) echo "1.4.2" ; exit 0 ;;
  supports)  ;;
  *) echo "usage: testsolver7 [--name|--version|supports <flag>]" >&2; exit 2 ;;
esac
...
chmod +x "$SOLVER_BIN_DIR/testsolver7"
echo "installed testsolver7 1.4.2 into $SOLVER_BIN_DIR"
